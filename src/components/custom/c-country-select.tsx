...
export function CCountrySelect({ continents, placeholder, ...rest }: CCountrySelectProps) {
    return (
        <Select {...rest}>
            <SelectTrigger
                id="select-37"
                className="p-[1rem] h-[58px] [&>span]:flex [&>span]:items-center [&>span]:gap-2 [&>span_svg]:shrink-0 [&>span_svg]:text-muted-foreground/80"
            >
                    ...
                    <SelectGroup key={continent.continent} className="p-[10px]">
                        ...
                        {continent.countries.map((item) => (
                            <SelectItem key={item.value} value={item.value} className="p-[10px]">
                                <span className="lg:text-base">{getOptionLabel(item)}</span>
                            </SelectItem>
                        ))}
                    </SelectGroup>
                ))}
